...
<!-- 
 ...
 -->
<head>
	<title>Kitchen Requests Admin</title>
	<meta charset="utf-8"/>
</head>
<body>
	...
	<table border="1">
		...
		<c:forEach var="req" items="${requests}">
	  		<tr>
	  			<td>${req.value.name} 
	  				<form name="freezeForm" action="/Final_Project_Kimpel/RequestServlet" method="get">
	  					<input name="op" type="hidden" value="freeze">
	  					<input name="name" type="hidden" value="${req.value.name}"/>
	  					<c:choose>
	  						<c:when test="${req.value.frozen}">
	  							<input value="UnFreeze" type="submit"/>
	  						</c:when>
	  						<c:otherwise>
	  							<input value="Freeze" type="submit"/>
	  						</c:otherwise>
	  					</c:choose>
	  				</form>
	  			</td>
